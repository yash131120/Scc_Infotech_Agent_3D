  ...
  return (
    <Float speed={1.4} rotationIntensity={0.5} floatIntensity={1}>
      <group ref={meshRef} position={position} onClick={onClick}>
        ...
        {/* Avatar */}
        <RoundedBox args={[1.2, 1.2, 0.1]} position={[0, 0, 0.15]} radius={0.2} smoothness={4}>
...
const TeamCard = ({ member, index, isActive, onClick }: any) => {
  return (
    ...
    >
      <div className={`relative transform-gpu transition-all duration-500 preserve-3d ${
        ...
        <div className={`backdrop-blur-md rounded-3xl p-8 border transition-all duration-500 ${
          ...
          <div className="relative mb-6">
            <div className={`w-24 h-24 mx-auto rounded-full flex items-center justify-center transition-all duration-300 ${
              isActive ? 'bg-[#12A594] ring-4 ring-[#12A594]/30' : 'bg-white/10'
            }`}>
            ...
            {/* Status indicator */}
            <div className={`absolute -bottom-1 -right-1 w-6 h-6 rounded-full border-4 border-white/20 ${
  ...
  const teamMembers = [
    {
      ...
      skills: ['Leadership', 'Strategy', 'Mobile Development', 'Business Development'],
      status: 'online',
